...
import { Download, FileText, X, Bus, Clock, MapPin, ArrowRight } from "lucide-react";
            ...
            <>
              <div className="relative rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
                ...
                <div className="mb-4 flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-bold text-gray-900">Route {route.routeNo}</h3>
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex items-center gap-2 text-gray-600">
                    ...
                    <span>Bus: <span className="font-medium text-gray-900">{selected.bus?.busNumber}</span></span>
                  </div>
                  {route.via && (
                    <div className="flex items-center gap-2 text-gray-600">
                      <MapPin className="h-4 w-4" />
                      <span>Via: <span className="font-medium text-gray-900">{route.via}</span></span>
                    </div>
                  )}
                  <div className="flex items-center gap-2 text-gray-600">
                    <Clock className="h-4 w-4" />
                    <span>
                      {selected.firstBus} — {selected.lastBus} ({selected.frequency})
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-xl border border-gray-200 bg-white shadow-sm">
                <RouteTimeline stops={route.stops} firstBus={selected.firstBus} />
              </div>
            </>
          ) : (
            <>
              ...
              <div className="rounded-xl border border-green-100 bg-green-50 p-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="text-sm font-bold text-gray-900">Download Full Schedule</h4>
                    <p className="text-xs text-gray-500">PDF Format</p>
                  </div>
                </div>
                <button className="flex h-8 w-8 items-center justify-center rounded border border-green-200 bg-white text-green-600 transition-colors hover:bg-green-100">
